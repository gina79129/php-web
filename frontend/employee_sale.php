<div style="width:70px;background-color:#FFF0F5;text-align:center;margin:20px 0 0 20px;  box-shadow:2px 2px 2px rgba(20%,20%,40%,0.5);padding:3px; border-radius:30px;"><a href="index.php?do=content&repo=empsale89">89年度</a></div>
<hr>
<?php
...
<table class="empsaletable">
   ...
   <tr><td  style="border:unset;"><?echo date("Y-m-d");?></td><td  style="border:unset;" colspan="3"><h2>民國90年產品銷售狀況</h2></td><td  style="border:unset;">葉珏汝</td></tr>
...
$sqltotal="SELECT `業務姓名`, `customer`.`客戶寶號`,`customer` . `聯絡人`, sum( (`數量`)*(`單價`)) as '總額' FROM `sales2` JOIN `customer` on `sales2`.`客戶代號` = `customer`. `客戶代號` JOIN `product` on `sales2`. `產品代號` = `product`. `產品代號` where `交易年` = '90' and `業務姓名` ='$Employeename' group by `業務姓名` , `客戶寶號` order by `總額` desc";
